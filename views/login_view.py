...
class LoginView(ctk.CTk):
    def __init__(self, switch_to_register_callback=None):
        super().__init__()
        ctk.set_appearance_mode("dark")
        ctk.set_default_color_theme("blue")
        self.title("Iniciar Sesión - League of Legends")
        ...
        self.title_label = ctk.CTkLabel(self.main_frame, text="INICIAR SESIÓN", font=self.fonts["title"],
        ...
        self.username_label.grid(row=1, column=0, pady=(20, 5), sticky="w", padx=30)
        ...
        self.password_label = ctk.CTkLabel(self.main_frame, text="Contraseña", font=self.fonts["label"],
                                           text_color=self.styles.COLORS["text"])
        self.password_label.grid(row=3, column=0, pady=(15, 5), sticky="w", padx=30)
        self.password_entry = ctk.CTkEntry(self.main_frame, height=40, corner_radius=10,
                                           ...
                                           text_color=self.styles.COLORS["text"])
        self.password_entry.grid(row=4, column=0, pady=5, padx=30, sticky="ew")
        
        self.login_button = ctk.CTkButton(self.main_frame, text="INICIAR SESIÓN", command=self.login_callback,
        ...
        self.status_label.grid(row=6, column=0, pady=(10, 10))
        
        self.register_link = ctk.CTkButton(self.main_frame, text="¿No tienes cuenta? Regístrate",
                                           command=self.switch_to_register, font=self.fonts["label"],
                                           fg_color="transparent", text_color=self.styles.COLORS["accent"],
                                           hover_color=self.styles.COLORS["frame"])
        self.register_link.grid(row=7, column=0, pady=10)
    ...
    def login_callback(self):
        ...
        if not username or not password:
            self.status_label.configure(text="Por favor, complete todos los campos.")
            return
        
        self.status_label.configure(text="Cargando...")
        self.login_button.configure(state="disabled")
        
        thread = threading.Thread(target=self._login_request, args=(username, password))
        thread.start()
    
    def _login_request(self, username, password):
        try:
            response = requests.post("http://localhost:5000/login", 
                                    json={"summoner_name": username, "password": password}, timeout=5)
            data = response.json()
            message = data["message"] if response.status_code == 200 else data["error"]
    ...
    def on_resize(self, event):
        ...
        self.login_button.configure(width=button_width)
